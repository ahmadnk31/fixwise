    ...
    try {
        ...
        <html>
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e5e7eb;">
              <h2 style="color: #1A2332; margin-top: 0;">
                ${data.status === "confirmed" ? "✅ Booking Confirmed!" : data.status === "cancelled" ? "❌ Booking Cancelled" : "✅ Booking Completed"}
              </h2>
...
async function PATCH(request, { params }) {
    try {
        ...
        const { data: currentBooking } = await supabase.from("bookings").select(`
        *,
      ...
      `).eq("id", id).single();
        ...
        if (status && status !== currentBooking.status) {
            const shop = currentBooking.shop;
            if (status === "pending") {
                ...
                (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$email$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sendBookingStatusUpdateEmail"])({
                    bookingId: booking.id,
                    userName: booking.user_name,
                    userEmail: booking.user_email,
                    shopName: shop?.name || "Repair Shop",
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1b8a351f._.js.map